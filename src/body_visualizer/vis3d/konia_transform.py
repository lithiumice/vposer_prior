...
def rotation_matrix_to_quaternion(
    ...
    if order == QuaternionCoeffOrder.XYZW:
        warnings.warn("`XYZW` quaternion coefficient order is deprecated and" " will be removed after > 0.6. " "Please use `QuaternionCoeffOrder.WXYZ` instead.")
...
def quaternion_to_rotation_matrix(quaternion: torch.Tensor, order: QuaternionCoeffOrder = QuaternionCoeffOrder.WXYZ) -> torch.Tensor:
...
def quaternion_exp_to_log(
    ...
    # apply log map
    quaternion_log: torch.Tensor = quaternion_vector * torch.acos(torch.clamp(quaternion_scalar, min=-1.0 + eps, max=1.0 - eps)) / norm_q
