...
def exprdir2model(expr_dir):
    ...
    assert len(available_ckpts) > 0, ValueError(
        "No checck points found at {}".format(model_snapshots_dir)
    )
    trained_weigths_fname = available_ckpts[-1]

    model_ps_fname = glob.glob(
        osp.join("/", "/".join(trained_weigths_fname.split("/")[:-2]), "*.yaml")
    )
...
def load_model(
    expr_dir,
    model_code=None,
    remove_words_in_model_weights=None,
    load_only_ps=False,
    disable_grad=True,
    custom_ps=None,
):
    ...
    wts_in_model_not_in_file = set(instance_model_keys).difference(
        set(trained_model_keys)
    )
    ## keys that are in the current model not in the training weights
    wts_in_file_not_in_model = set(trained_model_keys).difference(
        set(instance_model_keys)
    )
